...
if (!isset($conn)) {
    ...
    header("Location: ../dashboard/index.php?page=messages&lang=$lang$oid"); exit;
}
...
$stmt = $conn->prepare("SELECT o.id, o.order_number, o.service_title, o.status,
    u.first_name, u.last_name, u.id as provider_user_id,
    ...
    WHERE o.customer_id=? AND o.status IN ('accepted','in_progress','completed','requested')
...
if ($sel_id) {
    $stmt = $conn->prepare("SELECT o.*, u.first_name, u.last_name, u.id as provider_user_id
        FROM orders o
        JOIN service_providers sp ON o.provider_id=sp.id
        JOIN users u ON sp.user_id=u.id
        ...
        <div class="msg-sidebar">
            <?php if (empty($conversations)): ?>
                <div class="text-center p-3">
                    ...
                    <p class="text-muted small">No conversations yet. Order a service to start chatting.</p>
                </div>
            <?php else: ?>
                ...
                <div class="msg-footer">
                    <div class="d-flex gap-2 align-items-end">
                        ...
                        <div class="d-flex flex-column gap-1">
                            ...
                            <button class="btn btn-sm btn-primary" onclick="sendMsg()">
            ...
            <?php else: ?>
                <div class="d-flex align-items-center justify-content-center h-100 text-center p-4">
                    <div>
                        ...
                        <p class="text-muted small">Choose a provider from the left to start messaging.</p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
...
(function(){
    ...
    const receiverId = <?php echo $sel['provider_user_id']; ?>;
    const lang       = '<?php echo $lang; ?>';
    const url        = 'index.php?page=messages&lang=' + lang + '&order_id=' + orderId;
...
window.selectConv = function(id) {
    history.pushState({page:'messages',extra:{}}, '', 'index.php?page=messages&lang=<?php echo $lang; ?>&order_id='+id);
    const mc = document.getElementById('mainContent');
    mc.innerHTML = '<div class="text-center py-5"><i class="fas fa-spinner fa-spin fa-2x text-muted"></i></div>';
    fetch('index.php?page=messages&lang=<?php echo $lang; ?>&order_id='+id, {headers:{'X-Requested-With':'XMLHttpRequest'}})
